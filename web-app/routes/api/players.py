...
@players_bp.route("/match-detail/<int:match_id>/<player_id>")
def match_detail(match_id, player_id):
    """Get detailed match info including Omens point breakdown if applicable."""
    player_id_str = str(player_id)
    if player_id_str.startswith("google_"):
    ...
    try:
        service = MatchService()
        match = service._match_repo.get_match_by_id(match_id)
        if not match:
            return jsonify({"error": "Match not found"}), 404
        ...
        if pid == winner_id:
            did_win = True
            deck_json_str = match.get("winner_json")
            opp_deck_json_str = match.get("loser_json")
        elif pid == loser_id:
            did_win = False
            deck_json_str = match.get("loser_json")
            opp_deck_json_str = match.get("winner_json")
        else:
        ...
        result = {
            ...
            "player_name": match["winner_name"] if did_win else match["loser_name"],
            "opponent_name": match["loser_name"] if did_win else match["winner_name"],
            "match_type": match.get("match_type", "ranked"),
        }

        # Extract avatar/elements from both decks
        for key, json_str in [("player_deck", deck_json_str), ("opponent_deck", opp_deck_json_str)]:
            avatar = None
            elements = []
                    ...
                    for section in ("spellbook", "atlas", "sideboard"):
                        for card in deck.get(section, []) or []:
                            ...
                            if el:
                                for e in el.split(","):
                                    e = e.strip()
                                    if e and e not in elements:
                                        elements.append(e)
                except (json.JSONDecodeError, KeyError):
                    pass
            result[f"{key}_avatar"] = avatar
            result[f"{key}_elements"] = elements

        # Omens points breakdown
        if match.get("match_type") == "points":
            ...
            for key, json_str in [("player_omens", deck_json_str), ("opponent_omens", opp_deck_json_str)]:
                breakdown = {"total_points": 0, "max_budget": max_budget, "cards": []}
                if json_str and json_str != "{}":
                    try:
                        deck = json.loads(json_str)
                        for section in ("avatar", "spellbook", "atlas", "sideboard"):
                            for card in deck.get(section, []) or []:
                                ...
                                if pts > 0:
                                    card_total = pts * qty
                                    breakdown["total_points"] += card_total
                                    breakdown["cards"].append({
                                        "name": name,
                                        "quantity": qty,
                                        "points_each": pts,
                        ...
                        pass
                breakdown["cards"].sort(key=lambda c: -c["points_total"])
                breakdown["is_valid"] = breakdown["total_points"] <= max_budget
