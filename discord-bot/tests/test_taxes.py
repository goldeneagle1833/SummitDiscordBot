...
class TestTaxesCommandMessage:
    @pytest.mark.asyncio
    async def test_response_tags_fartlord_and_shows_gain(self, taxes_db):
        _seed_scores(
            [
                (42, "Filet Minyon", 1000),
                (2, "Second", 500),
                (3, "Third", 100),
            ]
        )
        ctx = MagicMock()
        ctx.author.id = 42
        ctx.author.global_name = "Filet Minyon"
        ctx.send = AsyncMock()
        taxes_db.update_fart_leader_role = AsyncMock()

        await FunCog.taxes.callback(taxes_db, ctx)

        msg = ctx.send.await_args.args[0]
        assert "<@42>" in msg
        assert "Filet Minyon" in msg
        assert "+120 points" in msg  # 20% of 500 + 20% of 100
        assert "TAXES COLLECTED" in msg
