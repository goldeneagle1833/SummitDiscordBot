...
class TestWinnerReport:
    ...
    async def test_winner_report_creates_record(self):
        ...
        with patch("utils.database.scrape_Curosa", return_value="{}"):
        ...
        assert row[1] == 111111111  # winner_id
        assert row[2] == "Winner"  # winner_display_name
        assert row[3] == 222222222  # losser_id
        assert row[4] == "Loser"  # losser_display_name

    @pytest.mark.asyncio
    async def test_winner_report_updates_elo(self):
        """Test that winner_report updates ELO ratings."""
        with patch("utils.database.scrape_Curosa", return_value="{}"):
            await winner_report(
                reporter_id=123456789,
                user_id=111111111,
...
class TestSoloMatchReport:
    ...
    async def test_solo_match_report_creates_record(self):
        ...
        assert row is not None
        assert row[0] == 123456789  # reporter_id
        assert row[1] == "TestPlayer"  # reporter_name
        assert row[2] == "OpponentName"  # opponent_name
        assert row[3] == 1  # is_winner (True)
...
class TestFunctionSignatures:
    ...
    def test_winner_report_signature(self):
        ...
        assert len(params) == 12, (
            f"winner_report should have 12 params, has {len(params)}"
        )
    ...
    def test_losser_report_signature(self):
        ...
        assert params == expected_params, f"Expected {expected_params}, got {params}"
        assert len(params) == 12, (
            f"losser_report should have 12 params, has {len(params)}"
        )
