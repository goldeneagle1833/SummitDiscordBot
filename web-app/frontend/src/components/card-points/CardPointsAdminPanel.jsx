...
import { fetchCardPointsAdmins, addCardPointsAdmin, removeCardPointsAdmin } from '@/api/cardPoints'

export default function CardPointsAdminPanel() {
  ...
  const handleAdd = async (e) => {
    ...
    setAddError(null)
    try {
      await addCardPointsAdmin(discordId.trim(), displayName.trim() || null)
      setDiscordId('')
      setDisplayName('')
      load()
    } catch (err) {
  ...
  const handleRemove = async (id) => {
    ...
    try {
      await removeCardPointsAdmin(id)
      load()
    } catch (err) {
  ...
  return (
    <div className="bg-bg-surface border border-border rounded-lg p-4">
      <h3 className="text-base font-semibold text-text-primary mb-3">Card Points Admins</h3>
      ...
      {loading ? (
        ...
        <div className="mb-4 space-y-1">
          {admins.length === 0 ? (
            <p className="text-sm text-text-muted">No Card Points admins yet.</p>
          ) : (
            admins.map((a) => (
